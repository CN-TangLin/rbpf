...
impl JitCompiler {
    ...
    fn emit_store(&self, mem: &mut JitMemory, size: OperandSize, src: u8, dst: u8, offset: i32) {
        match size {
            OperandSize::S16 => self.emit1(mem, 0x66),
            _ => {},
        };
    ...
    pub(crate) fn jit_compile(
        ...
        match (use_mbuff, update_data_ptr) {
            ...
            (true, true) => {
                self.emit_alu64(mem, 0x01, RDI, R8);
                self.emit_store(mem, OperandSize::S64, RDX, R8, 0);
                self.emit_load(mem, OperandSize::S64, RDX, R8, 0);
                self.emit_alu64(mem, 0x01, RCX, R8);
                self.emit_alu64(mem, 0x01, RDI, R9);
                self.emit_store(mem, OperandSize::S64, R8, R9, 0);
                if map_register(1) != RDI {
                    self.emit_mov(mem, RDI, map_register(1));
                }
            }
        ...
        while insn_ptr * ebpf::INSN_SIZE < prog.len() {
            ...
            let _ = match insn.opc {
                ...
                ebpf::LD_IND_B   => { self.emit_mov(mem, R10, R11); self.emit_alu64(mem, 0x01, src, R11); self.emit_load(mem, OperandSize::S8,  R11, RAX, insn.imm); }
                ebpf::LD_IND_H   => { self.emit_mov(mem, R10, R11); self.emit_alu64(mem, 0x01, src, R11); self.emit_load(mem, OperandSize::S16, R11, RAX, insn.imm); }
                ebpf::LD_IND_W   => { self.emit_mov(mem, R10, R11); self.emit_alu64(mem, 0x01, src, R11); self.emit_load(mem, OperandSize::S32, R11, RAX, insn.imm); }
                ebpf::LD_IND_DW  => { self.emit_mov(mem, R10, R11); self.emit_alu64(mem, 0x01, src, R11); self.emit_load(mem, OperandSize::S64, R11, RAX, insn.imm); }
                ebpf::LD_DW_IMM  => {
                    insn_ptr += 1;
                ...
                ebpf::ST_B_IMM   => self.emit_store_imm32(mem, OperandSize::S8,  dst, insn.off as i32, insn.imm),
                ebpf::ST_H_IMM   => self.emit_store_imm32(mem, OperandSize::S16, dst, insn.off as i32, insn.imm),
                ebpf::ST_W_IMM   => self.emit_store_imm32(mem, OperandSize::S32, dst, insn.off as i32, insn.imm),
                ebpf::ST_DW_IMM  => self.emit_store_imm32(mem, OperandSize::S64, dst, insn.off as i32, insn.imm),
                ...
                ebpf::ST_DW_REG  => self.emit_store(mem, OperandSize::S64, src, dst, insn.off as i32),
                ...
                ebpf::MUL32_IMM | ebpf::MUL32_REG | ebpf::DIV32_IMM | ebpf::DIV32_REG | ebpf::MOD32_IMM | ebpf::MOD32_REG =>
                ...
                ebpf::LSH32_REG  => { self.emit_mov(mem, src, RCX); self.emit_alu32(mem, 0xd3, 4, dst); }
                ebpf::RSH32_IMM  => self.emit_alu32_imm8(mem, 0xc1, 5, dst, insn.imm as i8),
                ebpf::RSH32_REG  => { self.emit_mov(mem, src, RCX); self.emit_alu32(mem, 0xd3, 5, dst); }
                ...
                ebpf::ARSH32_REG => { self.emit_mov(mem, src, RCX); self.emit_alu32(mem, 0xd3, 7, dst); }
                ebpf::LE         => {},
                ebpf::BE         => {
                    match insn.imm {
                        16 => { self.emit1(mem, 0x66); self.emit_alu32_imm8(mem, 0xc1, 0, dst, 8); self.emit_alu32_imm32(mem, 0x81, 4, dst, 0xffff); }
                        32 | 64 => {
                ...
                ebpf::MUL64_IMM | ebpf::MUL64_REG | ebpf::DIV64_IMM | ebpf::DIV64_REG | ebpf::MOD64_IMM | ebpf::MOD64_REG =>
                    self.emit_muldivmod(mem, insn_ptr as u16, insn.opc, src, dst, insn.imm),
                ...
                ebpf::LSH64_REG  => { self.emit_mov(mem, src, RCX); self.emit_alu64(mem, 0xd3, 4, dst); },
                ebpf::RSH64_IMM  => self.emit_alu64_imm8(mem, 0xc1, 5, dst, insn.imm as i8),
                ebpf::RSH64_REG  => { self.emit_mov(mem, src, RCX); self.emit_alu64(mem, 0xd3, 5, dst); },
                ...
                ebpf::ARSH64_REG => { self.emit_mov(mem, src, RCX); self.emit_alu64(mem, 0xd3, 7, dst); },
                ebpf::JA         => self.emit_jmp(mem, target_pc),
                ebpf::JEQ_IMM    => { self.emit_cmp_imm32(mem, dst, insn.imm); self.emit_jcc(mem, 0x84, target_pc); }
                ebpf::JEQ_REG    => { self.emit_cmp(mem, src, dst); self.emit_jcc(mem, 0x84, target_pc); }
                ebpf::JGT_IMM    => { self.emit_cmp_imm32(mem, dst, insn.imm); self.emit_jcc(mem, 0x87, target_pc); }
                ebpf::JGT_REG    => { self.emit_cmp(mem, src, dst); self.emit_jcc(mem, 0x87, target_pc); }
                ebpf::JGE_IMM    => { self.emit_cmp_imm32(mem, dst, insn.imm); self.emit_jcc(mem, 0x83, target_pc); }
                ebpf::JGE_REG    => { self.emit_cmp(mem, src, dst); self.emit_jcc(mem, 0x83, target_pc); }
                ebpf::JLT_IMM    => { self.emit_cmp_imm32(mem, dst, insn.imm); self.emit_jcc(mem, 0x82, target_pc); }
                ebpf::JLT_REG    => { self.emit_cmp(mem, src, dst); self.emit_jcc(mem, 0x82, target_pc); }
                ebpf::JLE_IMM    => { self.emit_cmp_imm32(mem, dst, insn.imm); self.emit_jcc(mem, 0x86, target_pc); }
                ebpf::JLE_REG    => { self.emit_cmp(mem, src, dst); self.emit_jcc(mem, 0x86, target_pc); }
                ebpf::JSET_IMM   => { self.emit_alu64_imm32(mem, 0xf7, 0, dst, insn.imm); self.emit_jcc(mem, 0x85, target_pc); }
                ebpf::JSET_REG   => { self.emit_alu64(mem, 0x85, src, dst); self.emit_jcc(mem, 0x85, target_pc); }
                ebpf::JNE_IMM    => { self.emit_cmp_imm32(mem, dst, insn.imm); self.emit_jcc(mem, 0x85, target_pc); }
                ebpf::JNE_REG    => { self.emit_cmp(mem, src, dst); self.emit_jcc(mem, 0x85, target_pc); }
                ebpf::JSGT_IMM   => { self.emit_cmp_imm32(mem, dst, insn.imm); self.emit_jcc(mem, 0x8f, target_pc); }
                ebpf::JSGT_REG   => { self.emit_cmp(mem, src, dst); self.emit_jcc(mem, 0x8f, target_pc); }
                ebpf::JSGE_IMM   => { self.emit_cmp_imm32(mem, dst, insn.imm); self.emit_jcc(mem, 0x8d, target_pc); }
                ebpf::JSGE_REG   => { self.emit_cmp(mem, src, dst); self.emit_jcc(mem, 0x8d, target_pc); }
                ebpf::JSLT_IMM   => { self.emit_cmp_imm32(mem, dst, insn.imm); self.emit_jcc(mem, 0x8c, target_pc); }
                ebpf::JSLT_REG   => { self.emit_cmp(mem, src, dst); self.emit_jcc(mem, 0x8c, target_pc); }
                ebpf::JSLE_IMM   => { self.emit_cmp_imm32(mem, dst, insn.imm); self.emit_jcc(mem, 0x8e, target_pc); }
                ebpf::JSLE_REG   => { self.emit_cmp(mem, src, dst); self.emit_jcc(mem, 0x8e, target_pc); }
                ebpf::JEQ_IMM32  => { self.emit_cmp32_imm32(mem, dst, insn.imm); self.emit_jcc(mem, 0x84, target_pc); }
                ebpf::JEQ_REG32  => { self.emit_cmp32(mem, src, dst); self.emit_jcc(mem, 0x84, target_pc); }
                ebpf::JGT_IMM32  => { self.emit_cmp32_imm32(mem, dst, insn.imm); self.emit_jcc(mem, 0x87, target_pc); }
                ebpf::JGT_REG32  => { self.emit_cmp32(mem, src, dst); self.emit_jcc(mem, 0x87, target_pc); }
                ebpf::JGE_IMM32  => { self.emit_cmp32_imm32(mem, dst, insn.imm); self.emit_jcc(mem, 0x83, target_pc); }
                ebpf::JGE_REG32  => { self.emit_cmp32(mem, src, dst); self.emit_jcc(mem, 0x83, target_pc); }
                ebpf::JLT_IMM32  => { self.emit_cmp32_imm32(mem, dst, insn.imm); self.emit_jcc(mem, 0x82, target_pc); }
                ebpf::JLT_REG32  => { self.emit_cmp32(mem, src, dst); self.emit_jcc(mem, 0x82, target_pc); }
                ebpf::JLE_IMM32  => { self.emit_cmp32_imm32(mem, dst, insn.imm); self.emit_jcc(mem, 0x86, target_pc); }
                ebpf::JLE_REG32  => { self.emit_cmp32(mem, src, dst); self.emit_jcc(mem, 0x86, target_pc); }
                ebpf::JSET_IMM32 => { self.emit_alu32_imm32(mem, 0xf7, 0, dst, insn.imm); self.emit_jcc(mem, 0x85, target_pc); }
                ebpf::JSET_REG32 => { self.emit_alu32(mem, 0x85, src, dst); self.emit_jcc(mem, 0x85, target_pc); }
                ebpf::JNE_IMM32  => { self.emit_cmp32_imm32(mem, dst, insn.imm); self.emit_jcc(mem, 0x85, target_pc); }
                ebpf::JNE_REG32  => { self.emit_cmp32(mem, src, dst); self.emit_jcc(mem, 0x85, target_pc); }
                ebpf::JSGT_IMM32 => { self.emit_cmp32_imm32(mem, dst, insn.imm); self.emit_jcc(mem, 0x8f, target_pc); }
                ebpf::JSGT_REG32 => { self.emit_cmp32(mem, src, dst); self.emit_jcc(mem, 0x8f, target_pc); }
                ebpf::JSGE_IMM32 => { self.emit_cmp32_imm32(mem, dst, insn.imm); self.emit_jcc(mem, 0x8d, target_pc); }
                ebpf::JSGE_REG32 => { self.emit_cmp32(mem, src, dst); self.emit_jcc(mem, 0x8d, target_pc); }
                ebpf::JSLT_IMM32 => { self.emit_cmp32_imm32(mem, dst, insn.imm); self.emit_jcc(mem, 0x8c, target_pc); }
                ebpf::JSLT_REG32 => { self.emit_cmp32(mem, src, dst); self.emit_jcc(mem, 0x8c, target_pc); }
                ebpf::JSLE_IMM32 => { self.emit_cmp32_imm32(mem, dst, insn.imm); self.emit_jcc(mem, 0x8e, target_pc); }
                ebpf::JSLE_REG32 => { self.emit_cmp32(mem, src, dst); self.emit_jcc(mem, 0x8e, target_pc); }
                ebpf::CALL       => {
                    match insn.src {
                        0x0 => {
                            if let Some(helper) = helpers.get(&(insn.imm as u32)) {
                                ...
                                Err(Error::other(format!("[JIT] Error: unknown helper function (id: {:#x})", insn.imm as u32)))?;
                            };
                        }
                        0x1 => {
                        ...
                        _ => {
                            Err(Error::other(format!("[JIT] Error: unexpected call type #{:?} (insn #{insn_ptr:?})", insn.src)))?;
                        }
                    }
                }
                ebpf::TAIL_CALL  => { unimplemented!() },
                ebpf::EXIT       => { self.emit1(mem, 0xc3); }
                _                => {
                    Err(Error::other(format!("[JIT] Error: unknown eBPF opcode {:#2x} (insn #{insn_ptr:?})", insn.opc)))?;
                }
            };
            insn_ptr += 1;
        }
